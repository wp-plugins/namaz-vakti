...
$nv = new Namaz( NV_PLUGIN_DIR . 'cache' . DIRECTORY_SEPARATOR, $hicriaylar, $ulke_dil_isimleri );
...
Class WP_Namazvakti
{
	...
	*/
	public function update_namazvakti_plugin()
	{
		...
		{
			add_option( NV_DB_DEFAULT_TOWN_NAME, 9351);
		}
		...
		{
			update_option( NV_DB_DEFAULT_TOWN_NAME, 9351);
		}
		
		
		if ( ! file_exists( NV_PLUGIN_DIR . DIRECTORY_SEPARATOR . 'cache' . DIRECTORY_SEPARATOR . 'v.' . NV_VERSION . '.version' ) )
		{
			$dir = glob( NV_PLUGIN_DIR . DIRECTORY_SEPARATOR . 'cache' . DIRECTORY_SEPARATOR . '*.*' );
			foreach( $dir as $cfile )
			{
				@unlink($cfile);
			}
			
			$fp = fopen( NV_PLUGIN_DIR . DIRECTORY_SEPARATOR . 'cache' . DIRECTORY_SEPARATOR . 'v.' . NV_VERSION . '.version', "w" );
			fclose($fp);
		}
	}
	...
	public function namazvakti_ajax()
	{
		switch($_POST['do'])
		{
			...
			case 'getTimes':
				...
				$_SESSION[NV_DB_DEFAULT_CITY_NAME] = $_POST['city'];
				$_SESSION[NV_DB_DEFAULT_TOWN_NAME] = $_POST['town'];
			
				$vakit = $this->nv->vakit($_POST['country'], $_POST['city'], $_POST['town'], 'json');
				echo $vakit;
				die();
			break;
		}
