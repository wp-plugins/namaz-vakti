...
Class NV_Widget extends WP_Widget
{
	...
	*/
	public function widget( $args, $instance )
	{
		...
		$db_sehirler = $this->nv->sehirler($varsayilan_ulke);
		
		if( $varsayilan_sehir == $varsayilan_ilce )
		{
			$yer = __($db_ulkeler['veri'][$varsayilan_ulke], 'namazvakti') . '<br>' . $db_sehirler['veri'][$varsayilan_sehir];
		} else {
			$db_ilceler = $this->nv->ilceler($varsayilan_sehir);
			
			$yer = $db_sehirler['veri'][$varsayilan_sehir] . '<br>' . $db_ilceler['veri'][$varsayilan_ilce];
		}
		...
		// vakitleri al!
		$vakit = $this->nv->vakit( $varsayilan_ulke, $varsayilan_sehir, $varsayilan_ilce );
        ...
        <div class="namazvakti">
            ...
            <div class="zaman">
                ...
                <div class="hicri"><?php echo $this->hicri_dil_duzeltmesi($vakit['hicritarih']); ?></div>
	...
	{
		$exp = explode( ' ', $tarih );
		return $exp[0] . ' ' . __(trim($exp[1]), 'namazvakti') . ' ' . $exp[2];
	}
}